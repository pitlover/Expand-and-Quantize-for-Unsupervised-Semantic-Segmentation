...
class LSTM_Encoder_Decoder(nn.Module):
    def __init__(self, size=[512, 4, 4], split=4, pred_step=4, device=0):
        ...
        self.output = make_layers(2 * self.channel, self.channel, kernel_size=1, stride=1, padding=0, norm=False,
                                  activation=True, is_relu=False)
        self.split = split
    ...
    def forward(self, x1, x2):
        init_hidden = (Variable(torch.zeros(2, x1.shape[0], self.lstm_size)).cuda(self.device),
                       Variable(torch.zeros(2, x1.shape[0], self.lstm_size)).cuda(self.device))
        x1_out = x1
        x2_out = x2
        x1_split = torch.stack(torch.split(x1, int(self.width / self.split), dim=3)).view(self.split, -1, 1,
                                                                                          self.lstm_size)
        x1_split_reversed = torch.stack(torch.split(x1, int(self.width / self.split), dim=3)).flip(dims=[4]).view(
            self.split, -1, 1, self.lstm_size)
        x2_split = torch.stack(torch.split(x2, int(self.width / self.split), dim=3)).view(self.split, -1, 1,
                                                                                          self.lstm_size)
        x2_split_reversed = torch.stack(torch.split(x2, int(self.width / self.split), dim=3)).flip(dims=[4]).view(
            self.split, -1, 1, self.lstm_size)
        ...
        x2_out = torch.cat(
            (de_out.view(-1, self.channel, self.height, int(self.width / self.split)).flip(dims=[3]), x2_out), 3)
        for i in range(self.pred_step + self.split - 1):
...
class ContextualAttention(nn.Module):
    def __init__(self, ksize=3, stride=1, rate=1, fuse_k=3, softmax_scale=10, fuse=False, two_input=True,
                 weight_func='cos', use_cuda=False, device_ids=None):
    ...
    def forward(self, left, right, mid, shortcut, mask=None):
        ...
        raw_l = [extract_image_patches(shortcut[0][i], ksizes=[kernel, kernel],
                                       strides=[self.rate * self.stride, self.rate * self.stride], rates=[1, 1],
                                       padding='same') for i in range(len(shortcut[0]))]  # [N, C*k*k, L]
        if self.two_input:
            raw_r = [extract_image_patches(shortcut[1][i], ksizes=[kernel, kernel],
                                           strides=[self.rate * self.stride, self.rate * self.stride], rates=[1, 1],
                                           padding='same') for i in range(len(shortcut[1]))]  # [N, C*k*k, L]
        ...
        # w shape: [N, C*k*k, L]
        left = extract_image_patches(left, ksizes=[self.ksize, self.ksize], strides=[self.stride, self.stride],
        ...
        if self.two_input:
            right = extract_image_patches(right, ksizes=[self.ksize, self.ksize], strides=[self.stride, self.stride],
                                          rates=[1, 1], padding='same')
            # w shape: [N, C, k, k, L]
        ...
        for xi, li, ri, batch_idx in zip(mid_groups, l_groups, r_groups, batch):
            ...
            if self.fuse:
                ...
                for i in range(len(yi)):
                    yi[i] = yi[i].view(1, 1, int_ls[2] * int_ls[3],
                                       int_mids[2] * int_mids[3])  # (B=1, I=1, H=32*32, W=32*32)
                    yi[i] = same_padding(yi[i], [k, k], [1, 1], [1, 1])
                    yi[i] = F.conv2d(yi[i], fuse_weight, stride=1)  # (B=1, C=1, H=32*32, W=32*32)
                    yi[i] = yi[i].contiguous().view(1, int_ls[2], int_ls[3], int_mids[2],
                                                    int_mids[3])  # (B=1, 32, 32, 32, 32)
            ...
            yi = [yi[i].view(1, int_mids[2] * int_ls[3], int_mids[2], int_mids[3]) for i in
                  range(len(yi))]  # (B=1, C=32*32, H=32, W=32)
            # softmax to match
            ...
            for i in range(len(shortcut[0])):
                ...
                y_l[i].append(
                    F.conv_transpose2d(yi[0], li_center, stride=self.rate, padding=1) / 4.)  # (B=1, C=128, H=64, W=64)
                if self.two_input:
                    y_r[i].append(F.conv_transpose2d(yi[1], ri_center, stride=self.rate,
...
class Codebook(nn.Module):
    ...
    def forward(self, z, only_loss=False, only_quantize=False):
        """
        only_update: only update the codebook, making the codebook close to features, while not quantizing the features.
        only_quantize: only quantize the features and update the network, while not updating the codebook.
        else: quantize the features, update the network and codebook at the same time.
        """
        ...
        if only_loss:
            q_loss = torch.mean((z_q - z.detach()) ** 2) + self.beta * torch.mean((z_q - z.detach()) ** 2)
            return q_loss

        elif only_quantize:
            # q_loss = self.beta * torch.mean((z_q.detach() - z)**2)
            z_q = z + (z_q - z).detach()
...
class Generator(nn.Module):
    def __init__(self, device=0, skip=[0, 1, 2, 3, 4], attention=[0, 1, 2, 3, 4]):
        ...
        self.CA = ContextualAttention(ksize=3, stride=1, rate=2, fuse_k=3, softmax_scale=10, fuse=True, two_input=False,
                                      use_cuda=True, device_ids=device)
        self.codebook = Codebook(num_codebook_vectors=8192, latent_dim=512, beta=0.25)
        ## Stage 1
        # 256*256*3
        ...
        # 4*4*1024
        self.feature_in = make_layers(1024, 512, kernel_size=1, stride=1, padding=0, norm=False, activation=True,
                                      is_relu=False)
        ...
        # 4*4*512
        self.feature_out = make_layers(512, 1024, kernel_size=1, stride=1, padding=0, norm=False, activation=True,
                                       is_relu=False)
        ...
        self.decoder_stage5 = nn.Sequential(
            ...
            identity_block([1024, 512, 512, 1024], is_relu=True),
            make_layers_transpose(1024, 1024, kernel_size=4, stride=2, padding=1, bias=False, norm=True,
                                  activation=True, is_relu=True)
        )
        ...
        self.decoder_stage2 = nn.Sequential(
            ...
            identity_block([256, 64, 64, 256], is_relu=True, norm=False),
            identity_block([256, 64, 64, 256], is_relu=True, norm=False),
            make_layers_transpose(256, 128, kernel_size=4, stride=2, padding=1, bias=False, norm=False, activation=True,
                                  is_relu=True)
        )
        ...
        self.skip1 = nn.ReLU()
        self.decoder_stage1 = make_layers_transpose(128, 64, kernel_size=4, stride=2, padding=1, bias=False, norm=False,
                                                    activation=True, is_relu=True)
    ...
    def forward(self, x1, x2=None, only_encode=False):
        ...
        # Encode x1
        x1, shortcut_x1 = self.encode(x1)  # x1: (B, 512, 4, 4)
        for i in range(6):
        ...
        # Encode x2
        x2, shortcut_x2 = self.encode(x2)  # x2: (B, 512, 4, 4)
        for i in range(6):
            shortcut[i].append(shortcut_x2[i])

        # Update Codebook
        q_loss_1 = self.codebook(x1, only_loss=True)
        q_loss_2 = self.codebook(x2, only_loss=True)
        q_loss = q_loss_1 + q_loss_2

        # Feature Extrapolate
        f_out, f1, f2 = self.BCT(x1, x2)  # f_out, f1, f2: (B, 512, 4, 4)

        # Quantize mid feature
        f_out = self.codebook(f_out, only_quantize=True)

        # q_loss = q_loss_1 + q_loss_2
        # print(f'q_loss_1: {q_loss_1:.2f} | q_loss_2: {q_loss_2:.2f} | q_loss_mid: {q_loss_mid:.2f}')
